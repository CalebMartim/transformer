// Multiplicando os últimos hidden states calculados para obter 
// Attention(Q, K, V)
...
void matmul_cpu(double *host_H, double *host_V, double *host_A, int C, int D, int n_heads) {
  for (int h = 0; h < n_heads; ++h) {
    for (int j = 0; j < D; ++j) {
      for (int i = 0; i < C; ++i) {
        double soma = 0;
        for (int idx = 0; idx < C; ++idx) {
          soma += host_H[(h * C * C) + (i * C) + idx] * host_V[(h * C * D) + (idx * D) + j];
        }
        host_A[(h * C * D) + (i * D) + j] = soma;
      }
    }
  }
}
...
struct MultiHeadAttention{
  ...
  void pass_embedding(double *E){
    ...
    // Pega os valores calculados (já testados):
    double *host_H = (double *) malloc(n_heads * C * C * sizeof(double)); // (C, C)
    cudaMemcpy(host_H, device_H, n_heads * C * C * sizeof(double), cudaMemcpyDeviceToHost);
    double *host_V = (double *) malloc(n_heads * C * D * sizeof(double)); // (C, D)
    cudaMemcpy(host_V, device_V, n_heads * C * D * sizeof(double), cudaMemcpyDeviceToHost);

    // Aloca espaço para colocar o resultado da multiplicação
    double *host_A = (double *) malloc(n_heads * C * D * sizeof(double));

    // Faz o cálcuo host_H * host_V na cpu 
    matmul_cpu(host_H, host_V, host_A, C, D, n_heads);

    // Copia o resultado de A obtido na gpu
    double *device_copy_A = (double *) malloc(n_heads * C * D * sizeof(double));
    cudaMemcpy(device_copy_A, device_A, n_heads * C * D * sizeof(double), cudaMemcpyDeviceToHost);
      
    // Faz a a verificação
    for (int h = 0; h < n_heads; ++h) {
      for (int i = 0; i < C; ++i) {
        for (int j = 0; j < D; ++j) {
          if (fabs(host_A[(h * C * D) + (i * D) + j] - device_copy_A[(h * C * D) + (i * D) + j]) > 1e-9) {
            printf("Deu errado\n");
            printf("Head: %d\n", h);
            printf("i: %d j: %d\n", i, j);
            printf("Valor na CPU: %lf\n", host_A[(h * C * D) + (i * D) + j]);
            printf("Valor na GPU: %lf\n", device_copy_A[(h * C * D) + (i * D) + j]);
            return;
          }
        }
      }
    ...
    printf("Não deu errado no cálculo de Attention(V, Q, K)!\n");
