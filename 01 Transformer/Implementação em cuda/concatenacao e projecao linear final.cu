// Concatenação e projeção final, finalizando a implementação
// do maksed multi-head self attention 
...
void concatena_cpu(double *host_A, double *host_concat, int C, int D, int n_heads){
  for (int h = 0; h < n_heads; ++h) {
    for (int i = 0; i < C; ++i) {
      for (int j = 0; j < D; ++j) {
        host_concat[(i * D * n_heads) + (h * D) + j] = host_A[(h * C * D) + (i * D) + j];
      }
    }
  }
}
...
void matmul_final_cpu(double *host_concat, double *host_W_O, double *host_multihead, int C, int d_model) {
  for (int j = 0; j < d_model; ++j) {
    for (int i = 0; i < C; ++i) {
      double soma = 0;
      for (int idx = 0; idx < d_model; ++idx) {
        soma += host_concat[(i * d_model) + idx] * host_W_O[(idx * d_model) + j]; 
      }
      host_multihead[(i * d_model) + j] = soma;
    }
  }
}
...
struct MultiHeadAttention{
  ...
  void pass_embedding(double *E){
    ...
    concatena_cpu(host_A, host_concat, C, D, n_heads);

    double *device_copy_concat = (double *) malloc(C * d_model * sizeof(double));
    cudaMemcpy(device_copy_concat, device_concat, C * d_model * sizeof(double), cudaMemcpyDeviceToHost);

    for (int i = 0; i < C; ++i) {
      for (int j = 0; j < d_model; ++j) {
        if (fabs(host_concat[(i * d_model) + j] - device_copy_concat[(i * d_model) + j]) > 1e-9) {
          printf("Deu errado\n");
          printf("i: %d j: %d\n", i, j);
          printf("Valor na CPU: %lf\n", host_concat[(i * d_model) + j]);
          printf("Valor na GPU: %lf\n", device_copy_concat[(i * d_model) + j]);
          return;
        }
      }
    }

    printf("Concatenação deu certo!\n");
    for (int i = 0; i < C; ++i) {
      for (int j = 0; j < d_model; ++j) {
    ...
    // --- Fim do teste de concatenação ---

    printf("\n");

    printf("Realizando projeção linear final... \n");
    ...
    double *host_multihead = (double *) malloc(C * d_model * sizeof(double));
    matmul_final_cpu(host_concat, host_W_O, host_multihead, C, d_model);

    double *device_copy_multihead = (double *) malloc(C * d_model * sizeof(double));
    cudaMemcpy(device_copy_multihead, device_multihead, C * d_model * sizeof(double), cudaMemcpyDeviceToHost);

    // TESTE FINAL
    for (int i = 0; i < C; ++i) {
      for (int j = 0; j < d_model; ++j) {
        if (fabs(host_multihead[(i * d_model) + j] - device_copy_multihead[(i * d_model) + j]) > 1e-9) {
          printf("Deu errado\n");
          printf("i: %d j: %d\n", i, j);
          printf("Valor na CPU: %lf\n", host_multihead[(i * d_model) + j]);
          printf("Valor na GPU: %lf\n", device_copy_multihead[(i * d_model) + j]);
          return;
        }
      }
    }
    printf("Masked Multi-Head Self Attention calculado com sucesso!!\n");
    for (int i = 0; i < C; ++i) {
      for (int j = 0; j < d_model; ++j) {
        printf("%lf ", host_multihead[(i * d_model) + j]);
      }
      printf("\n");
    }
    printf("Absolute Cinema.\n");
  }
};
